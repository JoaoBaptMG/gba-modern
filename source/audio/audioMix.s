...
@ void audioMix(s8* curMixBuffer, AudioChannel* channels)
@ r1: curMixBuffer, r2: channels
...
.clearLoop:
    stmia   r2!, {r4-r11}           @ clear 32 bytes (worth of 16 samples)
...
.forEachChannel:
    ...
    orrle   r1, r1, #1 << 31            @ set a flag on r1 (it's a pointer, it's free) to go back
...
.toPadBuffer:
    str     r9, [r1, #-16]              @ store a zero to the channel pointer
...
.finishProcessing:
    str     r5, [r1, #-12]              @ store the position back
...
.perFinalSampleQuad:
    ldmia   r2!, {r4-r5}            @ load 4 samples at once (they're interleaved)
    ...
    add     sp, sp, #BUFFER_SIZE*2  @ get back the stack pointer
    pop     {r4-r11, lr}
    bx      lr

@ this will treat the pitch-altered channels, which might be a lot slower
.abnormalSpeed:
    @ quick reference, r0 = final buffer, r1 = channel pointer, r2 = int buffer, r3 = channel count
    @ r4 = sound pointer, r5 = position, r6 = volume, r7 = ?, r8 = playing speed
    push    {r0-r1, r3}                 @ push some unnecessary registers right now
    mov     r7, #BUFFER_SIZE            @ set the number of samples to process
    ldmia   r4!, {r8-r9}                @ get the song length and song size
    mla     r0, r11, r7, r5             @ r1 = final position
    cmp     r0, r8, lsl #12             @ check if the final position is less than the length
...
.abnormalOptimized:
    ...
    ldrb    r1, [r4, r5, lsr #12]       @ load the second sample
    add     r5, r5, r11                 @ add the playing speed
    ldrb    r3, [r4, r5, lsr #12]       @ load the third sample
    orr     r0, r0, r3, lsl #16         @ put it on its place
    add     r5, r5, r11                 @ add the playing speed
    ...
    mul     r3, r0, r6                  @ modulate the even samples
    mul     r10, r1, r6                 @ modulate the odd samples
    ldmia   r2, {r0, r1}                @ load the 8-word data
    add     r0, r0, r3                  @ add the even samples
    add     r1, r1, r10                 @ add the odd samples
    stmia   r2!, {r0, r1}               @ put everything back
    subs    r7, r7, #4                  @ decrease the counter
...
.handleOverflow3:
    ...
    mov     r0, #1 << 7                 @ append a blank sample
    orr     r0, r0, #1 << 23            @ append a blank sample
...
.abnormalSpeedOverflow:
    ...
    pop     {r0, r1, r3}                @ pop the registers
