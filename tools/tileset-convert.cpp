    ...
    const std::array<Color, 256>& originalPalette)
{
    ...
    state.tiles.resize(state8bpp.tiles.size());
    for (std::size_t i = 0; i < state8bpp.tiles.size(); i++)
    {
        for (std::size_t j = 0; j < 4; j++)
        {
            ...
            // Since the first char is always empty, setting it to elsehow
            // doesn't make sense
            if (newId == 0) state.tiles[i][j] = 0;
        }
    }

    // Copy the flags as exactly same
    state.flags = state8bpp.flags;
